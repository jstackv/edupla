...
const assessmentQuestionSchema = new mongoose.Schema(
  {
    ...
    type: {
      type: String,
      enum: ["mcq", "true_false", "fill_gap", "matching", "open"],
      required: true,
    },
...
const maintenanceSchema = new mongoose.Schema(
  {
    ...
    message: {
      type: String,
      default:
        "We're performing scheduled maintenance to improve EDUPLA. We'll be back online shortly — thank you for your patience.",
    },
...
// Mirrors groupMessageSchema's media fields so voice notes / photos / files
// can be shared privately, not just text. `content` is only required for
// message_type='text' — voice/image/file messages carry their content in
// voice_url/file_url instead (enforced in the controller, not here, since
// Mongoose subdocument conditional-required is awkward with `default`).
...
const discussionGroupSchema = new mongoose.Schema(
  {
    ...
    teacher_id: {
      ...
      ref: "User",
      required: true,
    }, // creator — has full access, and is the team leader's DM recipient
...
classCollaborationSchema.index(
...
const teacherDirectMessageSchema = new mongoose.Schema(
  {
    ...
    class_id: {
      ...
      ref: "Class",
      required: true,
    }, // class that established the relationship at send-time
    sender_id: {
      type: mongoose.Schema.Types.ObjectId,
      ref: "User",
      required: true,
...
teacherDirectMessageSchema.index({
  teacher_id: 1,
  student_id: 1,
  created_at: 1,
});
...
teacherDmConversationStateSchema.index(
...
const ClassCollaboration = mongoose.model(
  "ClassCollaboration",
  classCollaborationSchema,
);
const DirectMessage = mongoose.model("DirectMessage", directMessageSchema);
const TeacherDirectMessage = mongoose.model(
  "TeacherDirectMessage",
  teacherDirectMessageSchema,
);
const TeacherDmConversationState = mongoose.model(
  "TeacherDmConversationState",
  teacherDmConversationStateSchema,
);
