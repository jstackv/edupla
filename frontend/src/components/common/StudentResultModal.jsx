          ...
          {Array.from({ length: 10 }).map((_, i) => (
            <span
              ...
              style={{
                ...
                background: ['#f59e0b', '#10b981', '#c2410c', '#ec4899', '#eab308'][i % 5],
  ...
  return (
    <div style={{ '--i': i, '--srm-a-color': color }} className="srm-attempt-card p-3 flex items-center gap-3 relative flex-wrap sm:flex-nowrap">
      ...
      <button
        ...
        style={{
          background: canViewResponse ? 'rgba(194, 65, 12,0.12)' : 'var(--surface-100)',
          color: canViewResponse ? '#c2410c' : 'var(--text-secondary)',
  ...
  return (
    <div className="srm-panel space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        <StatTile icon={Clock} color="#c2410c" value={`${a.duration_minutes ?? '—'} min`} label="Time limit" />
      ...
      {!a.expired && a.attempts.length > 0 && (
        <div className="sa-note p-3 rounded-xl text-sm flex items-start gap-2.5" style={{ background: 'rgba(194, 65, 12,0.08)', border: '1px solid rgba(194, 65, 12,0.2)' }}>
          <Lock className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: '#c2410c' }} />
...
function OverallPanel({ data }) {
  ...
  return (
    <div className="srm-panel space-y-4">
      <div className="srm-hero flex flex-col sm:flex-row items-center gap-5" style={{ '--srm-color': color }}>
        ...
        <div className="min-w-0 flex-1 text-center sm:text-left relative z-10">
          <h3 className="font-display font-bold text-lg flex items-center justify-center sm:justify-start gap-1.5" style={{ color: 'var(--text-primary)' }}>
            <Layers className="w-4.5 h-4.5" style={{ color: '#c2410c' }} /> Overall — {data.type_label}
          </h3>
          <MetaRow
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        <StatTile icon={Target} color="#c2410c" value={o.total_obtained != null ? `${Math.round(o.total_obtained)} / ${o.combined_max}` : '—'} label="Total obtained" />
        <StatTile icon={TrendingUp} color={color} value={o.percentage != null ? `${o.percentage}%` : '—'} label="Overall %" />
        <StatTile icon={Award} color="#c2410c" value={o.marks_on_mw != null ? `${Math.round(o.marks_on_mw)} / ${o.module_weight}` : '—'} label="On module weight" />
...
export default function StudentResultModal({ assessment, onClose }) {
  ...
  const tabs = useMemo(() => {
    ...
    return showOverall ? [...assessmentTabs, { key: 'overall', label: 'Overall', title: 'Overall', color: '#c2410c' }] : assessmentTabs;
  }, [data]);
