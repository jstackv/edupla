...
const TABS = [
  ...
  { key: 'move', label: 'Move Students', icon: ArrowRightLeft, accent: '#f97316' },
...
export default function ManageClassModal({
  ...
  const avatarFrom = cls ? '#0f766e' : '#f97316';
  ...
  return (
    <>
              ...
              return (
                ...
                >
                  ...
                  {key === 'move' && selected.size > 0 && (
                    <span className="mcm-count-badge" style={{ background: '#f97316' }}>{selected.size}</span>
                  )}
                </button>
              );
            })}
            ...
            <div key="overview" className="mcm-tab-panel" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10 }}>
                {[
                  ...
                  { icon: Award, label: 'RTQF Level', value: cls.level || '—', color: '#f97316', bg: '#ffedd5' },
              ...
              {targetClassId && (
                <div key={targetClassId} className="mcm-flow-chip" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 12, background: 'var(--surface-50)', border: '1px solid var(--card-border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
                    ...
                    <ArrowRight size={14} className="mcm-arrow-nudge" style={{ color: '#f97316' }} />
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#f97316' }}>{targetClass?.name}</span>
                  </div>
                </div>
              )}
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
                  ...
                  {filteredStudents.length > 0 && (
                    <button onClick={toggleAllFiltered} style={{ fontSize: 11, fontWeight: 600, color: '#f97316', background: 'none', border: 'none', cursor: 'pointer' }}>
                    ...
                    return (
                      ...
                      >
                        <span className="mcm-check" data-selected={isSel}>
                          {isSel ? <CheckSquare size={16} style={{ color: '#fb923c' }} /> : <Square size={16} style={{ color: 'var(--text-secondary)' }} />}
                        </span>
                ...
                .map(t => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSel ? <Check size={16} style={{ color: '#fb923c', flexShrink: 0 }} /> : null}
                    </div>
                  );
                })}
          </div>
      ...
      <Modal isOpen={coTeachModal} onClose={() => setCoTeachModal(false)} title={`Add Teachers — ${cls.name}`}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {coTeachSelected.size > 0 && (
            <p style={{ fontSize: 11, color: '#f97316' }}>{coTeachSelected.size} teacher{coTeachSelected.size !== 1 ? 's' : ''} selected</p>
          )}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 4 }}>
      ...
      <style>{`
        ...
        .mcm-student-row:hover { background: rgba(249,115,22,0.08); transform: translateX(2px); }
        ...
        .mcm-student-row:focus-visible {
          outline: none;
          box-shadow: 0 0 0 3px color-mix(in srgb, #f97316 35%, transparent);
        }
        .mcm-tab-btn:focus-visible { border-radius: 8px; }
        .mcm-student-row:focus-visible { border-color: color-mix(in srgb, #f97316 45%, var(--card-border)); }
