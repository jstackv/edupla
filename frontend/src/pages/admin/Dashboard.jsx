...
/* ── Radial ring ── */
function RadialRing({ pct = 0, color = '#f97316', size = 52, stroke = 5 }) {
...
export default function AdminDashboard() {
  ...
  const heroStats = [
    { icon:Users,         label:'Total Teachers',  value:counts.teachers||0,                color:'#f97316', bg:'#fff7ed', trend:12,   sparkData:sparkTeachers, to:'/admin/teachers' },
    ...
    { icon:Layers,        label:'Modules',         value:analytics?.modules?.total||0,      color:'#f97316', bg:'#fff7ed', trend:null, sparkData:sparkModules,  to:'/admin/classes'  },
  ...
  const donutSegments = [
    { label:'Teachers',    value:counts.teachers||1,    color:'#f97316' },
    { label:'Students',    value:counts.students||1,    color:'#10b981' },
    { label:'Classes',     value:counts.classes||1,     color:'#0ea5e9' },
    { label:'Modules',     value:analytics?.modules?.total||1, color:'#f97316' },
  ];
  ...
  const markSegments = [
    ...
    { label:'Good (≥60%)',      value:markDist.good||0,      color:'#f97316' },
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
      ...
      {/* ── Hero Banner ── */}
      <div style={{ borderRadius:20, padding:'22px 26px', position:'relative', overflow:'hidden', background:'linear-gradient(135deg, #431407 0%, #9a3412 50%, #ea580c 100%)', boxShadow:'0 8px 32px rgba(249,115,22,0.35)' }}>
        ...
        <div style={{ position:'absolute', bottom:-30, right:130, width:100, height:100, borderRadius:'50%', background:'rgba(249,115,22,0.15)', pointerEvents:'none' }} />
        <div style={{ position:'relative', display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16, flexWrap:'wrap' }}>
          <div style={{ flex:1, minWidth:200 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
              <div style={{ padding:'4px 10px', borderRadius:99, background:'rgba(255,255,255,0.12)', border:'1px solid rgba(255,255,255,0.15)', display:'flex', alignItems:'center', gap:5 }}>
                ...
                <span style={{ fontSize:10, fontWeight:700, color:'#fdba74', letterSpacing:'0.08em', textTransform:'uppercase' }}>Administrator</span>
              </div>
          ...
          <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
            {[
              { label:'Teachers', val:counts.teachers||0, color:'#fdba74', icon:Users },
              { label:'Students', val:counts.students||0, color:'#6ee7b7', icon:GraduationCap },
              { label:'Modules',  val:analytics?.modules?.total||0, color:'#fdba74', icon:Layers },
        ...
        <div className="card">
          ...
          {activeTab === 'teachers' && (
            <>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div style={{ display:'flex', gap:5, flexShrink:0 }}>
                            {classRow && <>
                              <span style={{ fontSize:11, padding:'2px 7px', borderRadius:6, background:'#fff7ed', color:'#ea580c', fontWeight:600 }}>{classRow.class_count}cls</span>
              ...
              <div style={{ marginTop:10, paddingTop:10, borderTop:'1px solid var(--card-border)' }}>
                <Link to="/admin/teachers" style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, fontWeight:600, color:'#f97316', textDecoration:'none' }}>
          ...
          {activeTab === 'students' && (
            <>
              ...
              <div style={{ marginTop:10, paddingTop:10, borderTop:'1px solid var(--card-border)' }}>
                <Link to="/admin/students" style={{ display:'flex', alignItems:'center', gap:4, fontSize:12, fontWeight:600, color:'#f97316', textDecoration:'none' }}>
                ...
                <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:12 }}>
                  {categoryData.map((c,i) => (
                    <span key={i} style={{ fontSize:10, padding:'3px 8px', borderRadius:99, fontWeight:700,
                      background:['#fff7ed','#ecfdf5','#fffbeb','#fff7ed'][i%4],
                      color:['#ea580c','#059669','#d97706','#ea580c'][i%4] }}>
                      {c._id || 'Other'}: {c.count}
                    </span>
                  ))}
                </div>
                  ...
                  {analytics.modules.recent.map((m,i) => (
                    <div key={m.id||i} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', borderRadius:10, transition:'background 0.15s' }}
                      ...
                      <div style={{ width:32, height:32, borderRadius:9, background:'#fff7ed', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                        <BookMarked size={14} style={{ color:'#f97316' }} />
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
        ...
        <div className="card">
          ...
          {analytics?.submissionTrend?.length > 0 && (
            <div style={{ marginTop:16, paddingTop:14, borderTop:'1px solid var(--card-border)' }}>
              ...
              <MiniBarChart data={analytics.submissionTrend} color="#f97316" />
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:14 }}>
        ...
        <div className="card">
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
            ...
            <Link to="/admin/classes" style={{ display:'flex', alignItems:'center', gap:2, fontSize:11, fontWeight:600, color:'#f97316', textDecoration:'none' }}>
          ...
          {!stats?.classesByTeacher?.length
            ...
            : <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                {stats.classesByTeacher.slice(0,5).map((row,i) => {
                  ...
                  const colors = ['#f97316','#10b981','#0ea5e9','#f59e0b','#ec4899'];
        ...
        <div className="card">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            {[
              ...
              { label:'Total Modules',  val:analytics?.modules?.total||0, color:'#f97316', bg:'#fff7ed', icon:Layers, desc:'Across all classes' },
        ...
        <div className="card">
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            {[
              ...
              { label:'Storage Used',    pct:43, color:'#f97316' },
          ...
          <div style={{ marginTop:16, paddingTop:14, borderTop:'1px solid var(--card-border)', display:'flex', flexDirection:'column', gap:5 }}>
            ...
            {[
              { label:'Manage Teachers', to:'/admin/teachers', icon:Users,         color:'#f97316' },
