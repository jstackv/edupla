...
const FEATURES = [
  { key: 'documentManagement',   icon: FolderKanban,    color: '#c2410c' },
  { key: 'assignmentWorkflow',   icon: ClipboardList,   color: '#c2410c' },
  { key: 'teacherManagement',    icon: UserCog,         color: '#0ea5e9' },
  { key: 'studentManagement',    icon: Users,           color: '#10b981' },
  { key: 'assessmentModules',    icon: ListChecks,      color: '#c2410c' },
  { key: 'onlineAssessments',    icon: Timer,           color: '#c2410c' },
...
const CURRICULUM_PIPELINE = [
  ...
  { key: 'modules',              icon: Boxes,      color: '#c2410c' },
];

// Assessment categories used inside a module
const ASSESSMENT_TYPES = [
  { code: 'FA', key: 'fa', color: '#c2410c' },
...
const STATS = [
  { key: 'students',  v: '2,400+', icon: Users,      c: '#c2410c' },
...
function MockupCard(props) {
  ...
  var s = {
    ...
    border: dark ? 'rgba(255,255,255,0.06)' : 'rgba(194, 65, 12,0.1)',
    tp:     dark ? '#f1f5f9' : '#0f172a',
    tm:     dark ? '#64748b' : '#94a3b8',
    cb:     dark ? 'rgba(255,255,255,0.04)' : 'rgba(248,250,255,0.9)',
  };
  var cards = [
    { icon: Users,         label: tr('landing.mockup.students'),  v: '247', c: '#c2410c', bg: 'rgba(194, 65, 12,0.1)' },
  ...
  return (
    <div style={{ borderRadius:20, overflow:'hidden', border:'1px solid '+s.border, background:s.bg, boxShadow:dark?'0 40px 100px rgba(0,0,0,0.7)':'0 40px 100px rgba(194, 65, 12,0.18)', fontFamily:"'Outfit',sans-serif" }}>
      <div style={{ height:34, background:dark?'rgba(255,255,255,0.02)':'rgba(194, 65, 12,0.03)', borderBottom:'1px solid '+s.border, display:'flex', alignItems:'center', gap:5, padding:'0 12px' }}>
        {['#ef4444','#f59e0b','#10b981'].map(function(c,i) { return <div key={i} style={{ width:9, height:9, borderRadius:'50%', background:c, opacity:0.7 }} />; })}
        <div style={{ margin:'0 auto', height:18, width:180, borderRadius:5, background:dark?'rgba(255,255,255,0.05)':'rgba(194, 65, 12,0.06)', display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
      ...
      <div style={{ display:'flex', height:330 }}>
        <div style={{ width:52, background:dark?'rgba(255,255,255,0.02)':'rgba(194, 65, 12,0.025)', borderRight:'1px solid '+s.border, display:'flex', flexDirection:'column', alignItems:'center', paddingTop:14, gap:5 }}>
          <div style={{ width:28, height:28, borderRadius:8, background:'linear-gradient(135deg,#c2410c,#c2410c)', display:'flex', alignItems:'center', justifyContent:'center', marginBottom:10 }}>
            <GraduationCap size={13} color="white" />
          </div>
          {[LayoutDashboard, BookOpen, ClipboardList, Megaphone, FileText].map(function(Icon,i) {
            return (
              <div key={i} style={{ width:32, height:32, borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', background:i===0?'rgba(194, 65, 12,0.15)':'transparent' }}>
                <Icon size={14} color={i===0?'#ea580c':s.tm} />
              </div>
            );
          })}
        </div>
        <div style={{ flex:1, padding:14, display:'flex', flexDirection:'column', gap:10 }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            ...
            <div style={{ width:26, height:26, borderRadius:8, background:'linear-gradient(135deg,#c2410c,#c2410c)', fontSize:9, fontWeight:800, color:'white', display:'flex', alignItems:'center', justifyContent:'center' }}>SK</div>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, flex:1 }}>
            <div style={{ padding:'9px 10px', borderRadius:11, background:s.cb, border:'1px solid '+s.border }}>
              ...
              {[
                { icon: ClipboardList, c: '#c2410c', t: tr('landing.mockup.mathHwGraded') },
            ...
            <div style={{ padding:'9px 10px', borderRadius:11, background:s.cb, border:'1px solid '+s.border }}>
              ...
              <div style={{ display:'flex', alignItems:'flex-end', gap:3, height:58 }}>
                {[40,65,30,85,55,90,70].map(function(h,i) {
                  return <div key={i} style={{ flex:1, borderRadius:'2px 2px 0 0', background:'linear-gradient(180deg,#c2410c,#c2410c)', height:h+'%', opacity:0.6+(i/7)*0.4 }} />;
                })}
              </div>
...
function FooterLink(props) {
  ...
  var tm = props.tm;
  function onEnter(e) {
    e.currentTarget.style.color = dark ? '#f97316' : '#9a3412';
...
function BottomLink(props) {
  ...
  var dark = props.dark;
  var tm = props.tm;
  function onEnter(e) { e.currentTarget.style.color = dark ? '#f97316' : '#9a3412'; }
...
export default function Landing() {
  ...
  var t = {
    ...
    bord:     dark ? 'rgba(255,255,255,0.07)' : 'rgba(194, 65, 12,0.12)',
    tp:       dark ? '#f1f5f9' : '#0f172a',
    tm:       dark ? '#64748b' : '#64748b',
    stripeBg: dark ? 'rgba(255,255,255,0.015)' : 'rgba(194, 65, 12,0.022)',
  };
  ...
  return (
    <div style={{ minHeight:'100vh', background:t.bg, fontFamily:"'Outfit',system-ui,sans-serif", color:t.tp, overflowX:'hidden' }}>
      ...
      <style>{`
        ...
        ::-webkit-scrollbar-thumb{background:#f9731620;border-radius:9px}
        ...
        .footer-newsletter-input:focus { box-shadow: 0 0 0 3px rgba(194, 65, 12,0.25); border-color:#c2410c !important; }
      ...
      <div style={{ position:'fixed', inset:0, pointerEvents:'none', zIndex:0 }}>
        <div style={{ position:'absolute', width:800, height:800, borderRadius:'50%', top:'-20%', left:'-15%', background:'radial-gradient(circle,rgba(194, 65, 12,0.18),transparent)', filter:'blur(120px)' }} />
        <div style={{ position:'absolute', width:600, height:600, borderRadius:'50%', top:'40%', right:'-10%', background:'radial-gradient(circle,rgba(194, 65, 12,0.15),transparent)', filter:'blur(100px)' }} />
      ...
      <nav style={{ position:'fixed', top:0, left:0, right:0, width:'100%', zIndex:100, backdropFilter:'blur(24px)', background:scrolled?(dark?'rgba(8,12,24,0.92)':'rgba(248,250,255,0.92)'):(dark?'rgba(8,12,24,0.55)':'rgba(248,250,255,0.55)'), borderBottom:'1px solid '+(scrolled?t.bord:'transparent'), transition:'all 0.3s' }}>
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 2rem', height:66, display:'flex', alignItems:'center', gap:32 }}>
            ...
            {NAV.map(function(item) {
              return (
                <a key={item.key} href={'#'+item.anchor}
                  ...
                  onMouseEnter={function(e){ e.currentTarget.style.color=dark?'#f97316':'#9a3412'; e.currentTarget.style.background=dark?'rgba(194, 65, 12,0.08)':'rgba(194, 65, 12,0.06)'; }}
          ...
          <div className="nav-ctas" style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <Link to="/login"
              ...
              onMouseEnter={function(e){ e.currentTarget.style.borderColor='#c2410c'; e.currentTarget.style.color='#c2410c'; }}
            ...
            <Link to="/login"
              style={{ padding:'9px 20px', borderRadius:10, background:'linear-gradient(135deg,#9a3412,#9a3412)', color:'white', fontWeight:700, fontSize:13, textDecoration:'none', display:'flex', alignItems:'center', gap:6, boxShadow:'0 4px 14px rgba(194, 65, 12,0.4)', transition:'all 0.25s' }}
              onMouseEnter={function(e){ e.currentTarget.style.transform='translateY(-1px)'; e.currentTarget.style.boxShadow='0 8px 22px rgba(194, 65, 12,0.5)'; }}
              onMouseLeave={function(e){ e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='0 4px 14px rgba(194, 65, 12,0.4)'; }}
        ...
        {mob && (
          <div style={{ padding:'12px 1.5rem 20px', borderTop:'1px solid '+t.bord, background:dark?'rgba(8,12,24,0.97)':'rgba(248,250,255,0.97)', display:'flex', flexDirection:'column', gap:2 }}>
            ...
            <Link to="/login" onClick={function(){ setMob(false); }} style={{ marginTop:10, padding:'12px', borderRadius:11, background:'linear-gradient(135deg,#9a3412,#9a3412)', color:'white', fontWeight:700, fontSize:14, textDecoration:'none', textAlign:'center' }}>{tr('landing.nav.getStartedFree')}</Link>
          </div>
        )}
      </nav>
      ...
      <div style={{ position:'relative', zIndex:1, paddingTop:66 }}>
        ...
        <section style={{ position:'relative', overflow:'hidden' }}>
          ...
          <div className="hero-mesh-a" style={{ position:'absolute', top:'-12%', left:'6%', width:520, height:520, borderRadius:'50%', background:'radial-gradient(circle,rgba(194, 65, 12,0.16),transparent 70%)', filter:'blur(90px)', pointerEvents:'none', zIndex:0 }} />
          ...
          <div style={{ maxWidth:1200, margin:'0 auto', padding:'clamp(3.5rem,8vw,6rem) 2rem 4rem', display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4rem', alignItems:'center', position:'relative', zIndex:1 }} className="hero-grid">
            <div className="fade-up">
              <div style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'6px 14px', borderRadius:100, background:dark?'rgba(194, 65, 12,0.12)':'rgba(194, 65, 12,0.07)', border:'1px solid '+(dark?'rgba(194, 65, 12,0.28)':'rgba(194, 65, 12,0.18)'), marginBottom:28 }}>
                <div style={{ width:7, height:7, borderRadius:'50%', background:'#34d399', animation:'glow 2s infinite' }} />
                <Sparkles size={12} color={dark?'#ea580c':'#9a3412'} />
                <span style={{ fontSize:12, fontWeight:600, color:dark?'#ea580c':'#9a3412', letterSpacing:'0.04em' }}>{tr('landing.hero.badge')}</span>
              </div>
              <h1 style={{ margin:0 }}>
              ...
              <div style={{ display:'flex', gap:12, flexWrap:'wrap', marginBottom:'2.5rem' }}>
                <Link to="/login" className="hero-cta-primary"
                  style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'14px 28px', borderRadius:13, background:'linear-gradient(135deg,#9a3412,#9a3412)', color:'white', fontWeight:700, fontSize:15, textDecoration:'none', boxShadow:'0 8px 28px rgba(194, 65, 12,0.45)', transition:'all 0.25s' }}
                  onMouseEnter={function(e){ e.currentTarget.style.transform='translateY(-2px)'; e.currentTarget.style.boxShadow='0 14px 40px rgba(194, 65, 12,0.55)'; }}
                  onMouseLeave={function(e){ e.currentTarget.style.transform='translateY(0)'; e.currentTarget.style.boxShadow='0 8px 28px rgba(194, 65, 12,0.45)'; }}
                ...
                <a href="#features"
                  ...
                  onMouseEnter={function(e){ e.currentTarget.style.borderColor='#c2410c'; e.currentTarget.style.transform='translateY(-2px)'; }}
              ...
              <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
                {[
                  ...
                  { icon:Lock,   label:tr('landing.hero.uptime'),         color:'#c2410c' },
            ...
            <div style={{ animation:'fadeUp 0.9s ease both' }}>
              ...
              <div className="hero-wordmark" style={{ textAlign:'center', fontFamily:"'Outfit',sans-serif", fontWeight:900, fontSize:'clamp(4.5rem,9vw,8.5rem)', letterSpacing:'-0.05em', lineHeight:1, color:dark?'rgba(255,255,255,0.09)':'rgba(154, 52, 18,0.11)', whiteSpace:'nowrap', userSelect:'none', marginBottom:'-1.2rem', position:'relative', zIndex:0 }}>Edupla</div>

              <div style={{ position:'relative', marginTop:'2.75rem' }}>
                <div style={{ position:'absolute', top:-30, right:-30, width:200, height:200, borderRadius:'50%', background:'radial-gradient(circle,rgba(194, 65, 12,0.25),transparent)', filter:'blur(50px)', pointerEvents:'none' }} />
                <div style={{ position:'absolute', top:'50%', left:'50%', width:400, height:400, transform:'translate(-50%,-50%)', borderRadius:'50%', border:'1px dashed '+(dark?'rgba(194, 65, 12,0.18)':'rgba(194, 65, 12,0.14)'), pointerEvents:'none' }} className="hero-ring-spin" />
                ...
                <div className="float2" style={{ position:'absolute', bottom:54, left:-36, padding:'9px 14px', borderRadius:12, background:dark?'rgba(194, 65, 12,0.12)':'rgba(194, 65, 12,0.07)', border:'1px solid rgba(194, 65, 12,0.25)', backdropFilter:'blur(12px)', display:'flex', alignItems:'center', gap:7, boxShadow:'0 8px 24px rgba(194, 65, 12,0.15)', zIndex:2 }}>
                  <Bell size={13} color="#c2410c" />
                  <span style={{ fontSize:11, fontWeight:700, color:dark?'#ea580c':'#9a3412', whiteSpace:'nowrap' }}>{tr('landing.hero.newSubmissions')}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section id="features" style={{ padding:'6rem 2rem' }}>
          <div style={{ maxWidth:1200, margin:'0 auto' }}>
            <div style={{ textAlign:'center', marginBottom:'4rem' }}>
              <Label icon={Layers} text={tr('landing.featuresSection.badge')} color="#9a3412" />
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:24 }} className="hero-grid">
              ...
              <div style={{ padding:'30px 28px', borderRadius:22, background:t.card, border:'1px solid '+t.bord, backdropFilter:'blur(16px)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20 }}>
                  <ListChecks size={18} color="#c2410c" />
        ...
        <section id="how-it-works" style={{ background:t.stripeBg, borderTop:'1px solid '+t.bord, borderBottom:'1px solid '+t.bord, padding:'6rem 2rem' }}>
          <div style={{ maxWidth:1100, margin:'0 auto' }}>
            <div style={{ textAlign:'center', marginBottom:'4rem' }}>
              <Label icon={Rocket} text={tr('landing.howItWorksSection.badge')} color="#9a3412" />
          ...
          <div style={{ maxWidth:740, margin:'0 auto' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
              {tr('landing.faqs', { returnObjects: true }).map(function(item, i) {
                return (
                  <div key={item.q} style={{ borderRadius:16, background:t.card, border:'1px solid '+(faq===i?'rgba(194, 65, 12,0.35)':t.bord), overflow:'hidden', transition:'border-color 0.2s' }}>
        ...
        <section style={{ padding:'2rem 2rem 5rem' }}>
          <div style={{ maxWidth:900, margin:'0 auto', padding:'clamp(3rem,6vw,5rem) clamp(2rem,5vw,4rem)', borderRadius:28, background:'linear-gradient(135deg,#2a0c03 0%,#2a0c03 30%,#9a3412 65%,#9a3412 100%)', textAlign:'center', position:'relative', overflow:'hidden', boxShadow:'0 32px 80px rgba(154, 52, 18,0.45)' }}>
        ...
        <footer style={{ borderTop:'1px solid '+t.bord, background:dark?'rgba(8,12,24,0.85)':'rgba(248,250,255,0.92)', backdropFilter:'blur(20px)', position:'relative', overflow:'hidden' }}>
          ...
          <div className="footer-accent-line" style={{ position:'absolute', top:0, left:0, right:0, height:2, background:'linear-gradient(90deg,#c2410c,#c2410c,#0ea5e9,#c2410c,#c2410c)', pointerEvents:'none' }} />

          {/* giant faded wordmark for depth */}
          <div className="footer-wordmark" style={{ position:'absolute', bottom:-6, left:'50%', transform:'translateX(-50%)', fontFamily:"'Outfit',sans-serif", fontWeight:900, fontSize:'clamp(6rem,18vw,15rem)', letterSpacing:'-0.05em', lineHeight:1, color:dark?'rgba(255,255,255,0.025)':'rgba(154, 52, 18,0.035)', whiteSpace:'nowrap', pointerEvents:'none', userSelect:'none', zIndex:0 }}>Edupla</div>

          <div className="footer-blob-a" style={{ position:'absolute', bottom:-100, left:'10%', width:400, height:400, borderRadius:'50%', background:'radial-gradient(circle,rgba(194, 65, 12,0.1),transparent)', filter:'blur(80px)', pointerEvents:'none' }} />
          <div className="footer-blob-b" style={{ position:'absolute', top:-60, right:'5%', width:300, height:300, borderRadius:'50%', background:'radial-gradient(circle,rgba(194, 65, 12,0.08),transparent)', filter:'blur(60px)', pointerEvents:'none' }} />
            ...
            <div style={{ borderBottom:'1px solid '+t.bord, padding:'3rem 2rem 2.5rem' }} className="footer-pad">
              <div className="footer-grid" style={{ maxWidth:1200, margin:'0 auto', display:'grid', gridTemplateColumns:'2.2fr 1fr 1fr 1.2fr', gap:'3rem' }}>
                ...
                <div className="footer-brand">
                  <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:20 }}>
                    <div style={{ width:40, height:40, borderRadius:13, background:'linear-gradient(135deg,#9a3412,#9a3412)', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 16px rgba(194, 65, 12,0.45)' }}>
                ...
                <div>
                  <p style={{ display:'flex', alignItems:'center', gap:7, fontWeight:700, fontSize:11, letterSpacing:'0.09em', textTransform:'uppercase', color:t.tp, opacity:0.55, margin:'0 0 20px' }}><Layers size={13} color="#c2410c" />{tr('landing.footer.product')}</p>
                ...
                <div>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14 }}>
                    <div style={{ width:32, height:32, borderRadius:9, background:'rgba(194, 65, 12,0.12)', border:'1px solid rgba(194, 65, 12,0.22)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      <Mail size={13} color="#c2410c" />
                    </div>
                ...
                <div className="footer-bottom-left" style={{ display:'flex', alignItems:'center', gap:14, flexWrap:'wrap' }}>
                  <p style={{ fontSize:13, color:t.tm, margin:0 }}>
                    ...
                    <a href={OWNER.portfolio} target="_blank" rel="noopener noreferrer"
                      ...
                      onMouseEnter={function(e){ e.currentTarget.style.color = dark ? '#f97316' : '#9a3412'; }}
        ...
        {showTopBtn && (
          <button
            ...
            style={{ position:'fixed', bottom:26, right:26, zIndex:90, width:46, height:46, borderRadius:14, background:'linear-gradient(135deg,#9a3412,#9a3412)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 12px 30px rgba(194, 65, 12,0.45)' }}
          >
