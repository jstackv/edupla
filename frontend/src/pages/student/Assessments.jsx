...
function statusInfo(a) {
  ...
  if (a.has_pending_grading) return { key: 'pending', label: 'Awaiting grading', color: '#c2410c', bg: 'rgba(194, 65, 12,0.14)' };
  if (a.not_yet_available) return { key: 'scheduled', label: 'Starts soon', color: '#c2410c', bg: 'rgba(194, 65, 12,0.14)' };
...
const MODULE_PALETTE = ['#c2410c', '#c2410c', '#ec4899', '#f59e0b', '#10b981', '#0ea5e9', '#f43f5e', '#14b8a6'];
...
function ExpiryChip({ expiresAt, expired }) {
  ...
  const { color, bg, label } = diff === 0
    ? { color: '#c2410c', bg: 'rgba(194, 65, 12,0.14)', label: 'Due today' }
    : diff <= 2
...
function StartsChip({ availableFrom }) {
  ...
  return (
    <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold flex-shrink-0" style={{ background: 'rgba(194, 65, 12,0.14)', color: '#c2410c' }}>
...
function ModalStatusPill({ data }) {
  ...
  else if (data.not_yet_available) { label = 'Starts soon'; color = '#c2410c'; bg = 'rgba(194, 65, 12,0.14)'; }
...
function CountdownShowcase({ target, locked, windowStart, windowEnd }) {
  ...
  const color = locked ? '#c2410c' : urgent ? '#ef4444' : soon ? '#f59e0b' : '#c2410c';
      ...
      {loading || !data ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <StatTile icon={Clock} color="#c2410c" value={`${data.duration_minutes} min`} label="Time limit" i={0} />
            ...
            {data.available_from && (
              <StatTile icon={CalendarClock} color="#c2410c" value={fmtDate(data.available_from)} label="Opens for starting" small i={4} />
            )}
          </div>

          {data.not_yet_available && (
            <div className="sa-note sa-note-violet p-3.5 rounded-xl text-sm flex items-start gap-2.5">
              <CalendarClock className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: '#c2410c' }} />
...
function CommandCenter({ assessments, loaded }) {
  ...
  const items = [
    { label: 'Assigned', value: cTotal, color: '#c2410c', icon: ClipboardCheck },
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 mb-6 relative">
        <div
          ...
          style={{ background: 'linear-gradient(135deg, #c2410c, #c2410c)' }}
        >
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center relative overflow-hidden">
          <div className="pointer-events-none absolute inset-0" style={{ background: 'radial-gradient(circle at 50% 0%, rgba(194, 65, 12,0.08), transparent 60%)' }} />
          <div className="sa-empty-orbit" aria-hidden="true">
            <span /><span /><span />
          </div>
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 relative" style={{ background: 'rgba(194, 65, 12,0.1)' }}>
            <Inbox className="w-8 h-8 assessment-icon-float" style={{ color: '#c2410c' }} />
          </div>
