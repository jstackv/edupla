...
const LEADER_COLORS = ['#ea580c', '#c2410c'];
const TEACHER_DM_COLORS = ['#ea580c', '#c2410c'];

const SENDER_COLORS = ['#38bdf8', '#34d399', '#fb923c', '#f472b6', '#fb923c', '#fbbf24', '#4ade80', '#60a5fa'];
...
function VoiceBubble({ url, duration, isMine, otherAccent }) {
  ...
  const barColor = isMine ? 'rgba(255,255,255,0.95)' : (otherAccent || '#fdba74');
  const barMuted = isMine ? 'rgba(255,255,255,0.32)' : `${otherAccent || '#fdba74'}50`;
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 224 }}>
      ...
      <button onClick={toggle} title={playing ? 'Pause' : 'Play'}
        ...
        style={{ background: isMine ? 'rgba(255,255,255,0.24)' : `${otherAccent || '#fb923c'}22`, color: isMine ? '#fff' : (otherAccent || '#fdba74'), '--voice-glow-color': isMine ? 'rgba(255,255,255,0.35)' : `${otherAccent || '#fb923c'}70` }}>
...
function MessageBubble({
  ...
  const otherAccent = isMine ? null : isTeacherMsg ? '#ea580c' : isGroupThread ? senderTint : DM_RECEIVED_ACCENT;
  const nameColor = isTeacherMsg ? '#ea580c' : isLeaderMsg ? GOLD : senderTint;
      ...
      <div style={{ maxWidth: '72%' }}>
        {item.isFirst && !item.isMine && item.author_name && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4, marginLeft: 4 }}>
            ...
            {isTeacherMsg && <span style={{ fontSize: 9, padding: '2px 6px', borderRadius: 10, fontWeight: 700, background: 'rgba(234,88,12,0.12)', color: '#ea580c' }}>Teacher</span>}
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: 'var(--card-bg)', zIndex: 40, display: 'flex', flexDirection: 'column', animation: 'slideInFromRight 0.18s ease both' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px' }}>
        ...
        {group.teacher_name && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 14, background: 'rgba(234,88,12,0.06)', border: '1.5px solid rgba(234,88,12,0.2)' }}>
            <div style={{ width: 42, height: 42, borderRadius: '50%', flexShrink: 0, background: 'linear-gradient(135deg, #ea580c, #c2410c)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 15 }}>{group.teacher_name[0].toUpperCase()}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 700, fontSize: 13.5, color: 'var(--text-primary)' }}>{group.teacher_name}</div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#ea580c', marginTop: 2 }}>Teacher</div>
            </div>
          </div>
        )}
      </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div className="wa-recording-panel">
              <button onClick={toggleAudioPreview} title={audioPlaying ? 'Pause' : 'Play'}
                ...
                style={{ width: 28, height: 28, background: 'rgba(251,146,60,0.16)', color: 'var(--wa-voice-accent)', flexShrink: 0 }}>
              ...
              <Waveform bars={previewBars} progress={audioDuration ? Math.min(previewTime / audioDuration, 1) : 0}
                color="var(--wa-voice-accent)" mutedColor="rgba(251,146,60,0.22)" playing={audioPlaying} />
  ...
  return (
    ...
    >
      <div className="relative flex-shrink-0">
        ...
        {entry.type === 'leaderdm' && (
          <div style={{ position: 'absolute', bottom: -2, right: -2, width: 16, height: 16, borderRadius: '50%', background: '#ea580c', border: '2px solid var(--card-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div onClick={e => e.stopPropagation()} className="fast-modal-sheet" style={{ maxWidth: 420 }}>
        ...
        {classes.length > 1 && (
          <div style={{ padding: '0 18px 10px', display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            {classes.map(c => (
              <button key={c.id} onClick={() => setClassId(c.id)} style={{ fontSize: 11.5, fontWeight: 700, padding: '5px 10px', borderRadius: 20, border: 'none', cursor: 'pointer', background: classId === c.id ? 'rgba(234,88,12,0.14)' : 'var(--surface-100)', color: classId === c.id ? '#ea580c' : 'var(--text-secondary)' }}>{c.name}</button>
            ))}
          </div>
        )}
    ...
    <div style={{ minHeight: 'calc(100vh - 120px)', padding: '16px' }}>
            ...
            {loading ? [0, 1, 2, 3, 4].map(i => <InboxRowSkeleton key={i} delay={i * 60} />)
              : filtered.length === 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '70px 24px', textAlign: 'center' }}>
                  <div style={{ width: 60, height: 60, borderRadius: 16, marginBottom: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(234,88,12,0.1)' }}><MessageSquare style={{ width: 28, height: 28, color: '#ea580c', opacity: 0.6 }} /></div>
          ...
          {selectedEntry ? (
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ textAlign: 'center', padding: '0 32px' }}>
                <div style={{ width: 68, height: 68, borderRadius: 20, margin: '0 auto 16px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, rgba(234,88,12,0.14), rgba(219,39,119,0.1))', animation: 'ibxIconFloat 4s ease-in-out infinite' }}><MessageCircle style={{ width: 32, height: 32, color: '#ea580c', opacity: 0.85 }} /></div>
      ...
      <style>{`
        ...
        :root {
          ...
          --wa-voice-accent: #fdba74;
          --wa-voice-accent-2: #fb923c;
          ...
          --ibx-header-glow-1: rgba(234,88,12,0.14);
          --ibx-header-glow-2: rgba(219,39,119,0.08);
          --ibx-header-text: #221c35;
          --ibx-header-icon-bg: rgba(234,88,12,0.10);
          --ibx-header-icon-border: rgba(234,88,12,0.18);
          --ibx-header-icon-color: #c2410c;
          --ibx-header-btn-bg: rgba(234,88,12,0.08);
          --ibx-header-btn-bg-hover: rgba(234,88,12,0.16);
          --ibx-header-btn-border: rgba(234,88,12,0.18);
          --ibx-header-search-bg: rgba(234,88,12,0.06);
          --ibx-header-search-border: rgba(234,88,12,0.16);
          --ibx-header-search-text: #221c35;
          --ibx-header-search-icon: rgba(34,28,53,0.45);
          --ibx-header-filter-active-bg: #ffffff;
          --ibx-header-filter-active-text: #c2410c;
          --ibx-header-filter-active-shadow: 0 2px 8px rgba(234,88,12,0.18);
          --ibx-header-filter-inactive-bg: rgba(234,88,12,0.08);
        ...
        [data-theme='dark'], .dark {
          ...
          --ibx-header-glow-1: rgba(234,88,12,0.4);
        ...
        @keyframes voicePlayPulse {
          0%, 100% { box-shadow: 0 0 0 0 var(--voice-glow-color, rgba(251,146,60,0.4)); }
        ...
        @keyframes avatarGlowPulse { 0%, 100% { box-shadow: 0 0 0 0 var(--glow-color, rgba(234,88,12,0.45)); } 50% { box-shadow: 0 0 0 6px rgba(234,88,12,0); } }
